...
Matrix = np.ndarray
def symplectic_transformation(M: Matrix, Omega: bool = False, no_flux_variables: int = None, tol: float = 1e-16) -> tuple[Matrix, Matrix]:
    """
    ...
    ----------
        ...
        no_flux_variables:
            Parameter to indicate how many flux variables we have. It is only usefull if Omega = True. By default it is none.
        tol:
    ...
    # - Normalize the matrix V such that J = V.T @ M @ V
        ...
        if no_flux_variables == None:
            raise TypeError('If Omega = True, you need to introduce the input no_flux_variables')
        
        # Add an aditional phase to the imaginary eigenvectors, if it is necessary, to to obtain a block diagonal V matrix
        for i in range(len(imag_eigval)):
            if np.allclose(sum(imag_eigvec[:no_flux_variables,i]).real, 0):
                imag_eigvec[:,i] = 1j * imag_eigvec[:,i]
        
        # Construct the basis change matrix V
        V = np.empty((2*len(imag_eigval) + len(zero_eigval), 0))

        V = np.hstack((V, imag_eigvec.real))
        V = np.hstack((V, imag_eigvec.imag))
        V = np.hstack((V, zero_eigvec))
        

        # Divide each eigenvector by sqrt(2/eigenvalue) such that J = V.T @ M @ V
        for i, eigval in enumerate(imag_eigval):
            V[:,i] = np.sqrt(2/eigval.imag) * V[:,i]
            V[:,i+len(imag_eigval)] = np.sqrt(2/eigval.imag) * V[:,i+len(imag_eigval)]

        # Verify that the matrix T satisies the conditioSn it must satisfy
...
def symplectic_transformation2(M: Matrix, Omega: bool = False, no_flux_variables: int = None, tol: float = 1e-16) -> tuple[Matrix, Matrix]:
    """
    Transform a square matrix M = JH (with H a positive semidefinite matrix and J the Symplectic matrix) to eigval*J = [[0,eigval*1],[-eigval*1,0]]
    such that eigval*J = inv(T) @ M @ T. If Omega = True, instead transform M antisymmetric matrix to J such that J = T.T @ M @ T.
    ...
    if Omega == False:
        ...
        # Construct the basis change matrix T
        T = np.empty((2*len(imag_eigval) + len(zero_eigval), 0))

        T = np.hstack((T, imag_eigvec.real))
        T = np.hstack((T, imag_eigvec.imag))
        T = np.hstack((T, zero_eigvec))

        # Normalize T under the symplectic inner product
        #for i in range(len(imag_eigval)):
        ...
        # Eigenvectors normalization under the symplectic inner product
        normal_T = np.empty((M.shape[0], 0))

        for i in range(len(imag_eigval)):

            # First eigenvalues
            alpha = T[:,i].T @ J @ T[:,i+len(imag_eigval)]
            normal_T = np.hstack((normal_T, (T[:,i].reshape(-1,1))/alpha)) 

            # Verify the orthonormalization of the term i
            #assert np.allclose(normal_T[:,i].T @ J @ normal_T[:,i+len(imag_eigval)], 1, rtol = tol), \
             #   "There is an error in the orthonormalization of an eigenvector from a purely imaginary eigenvalue"
            
        for i in range(len(imag_eigval)):

            # First eigenvalues
            alpha = T[:,i+len(imag_eigval)].T @ J @ T[:,i]
            normal_T = np.hstack((normal_T, (T[:,i+len(imag_eigval)].reshape(-1,1))/alpha)) 
        ...
        # Obtain and return the output matrix 
        M_out = np.linalg.pinv(normal_T) @ M @ normal_T
        return M_out, normal_T
